...
| 节点04 | 经销商囤货计划 | 经销商正常囤货计划，或为完成年度任务/返利达标。**需通过合理性判断后方可计入 Firm 段** | `Rev_F4 = 经销商囤货月数(X) × 月均销售额 - 完成任务的返利金额` | DCL-05 |

#### 节点04 — Rev_F4 合理性判断规则

Rev_F4 是 MCL-01 四条公式中唯一带有主观判断性的输入，必须在录入时同步记录合理性依据，系统自动进行四维交叉验证：

| 判断维度 | 绿灯（计入 Firm） | 黄灯（需说明，计入 Rolling） | 红灯（不得计入，退回） |
|---|---|---|---|
| **囤货月数 X** | X ≤ 1.0 个月 | 1.0 < X ≤ 2.0，需填写囤货理由 | X > 2.0，超出合理备货范围 |
| **现有库存 + X**（来自DCL-01） | 当前库存月数 + X ≤ 2.0 | 2.0 < 库存月数 + X ≤ 3.0 | 库存月数 + X > 3.0，渠道已积压 |
| **近3月出库趋势** | 出库量环比持平或增长 | 出库量环比下降 ≤ 15% | 出库量下降 > 15% 且 X > 1.0，量减但囤货增，逻辑矛盾 |
| **返利率**（返利÷囤货金额） | 返利率 ≤ 3% | 3% < 返利率 ≤ 8%，需说明返利政策 | 返利率 > 8%，实质为高返利驱动压货 |

**额外校验规则：**
- 上一期囤货消化率 < 70% → 当期 X 自动按消化率打折后计入 Rolling
- 单一经销商 Rev_F4 占全渠道 Rev_F4 > 40% → 触发集中度预警，需财务复核
- 囤货理由必须从枚举值中选择：`季节性备货 / 新品上市支持 / 年度任务冲刺 / 返利政策驱动 / 其他（需手工填写）`
