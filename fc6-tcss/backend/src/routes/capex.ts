...
router.get('/', authenticate, async (req, res) => {
  ...
  const records = await prisma.capexPlan.findMany({
...
router.post('/', authenticate, checkVersionAccess, async (req: AuthRequest, res) => {
  const record = await prisma.capexPlan.create({ data: req.body })
  res.status(201).json(record)
})

router.put('/:id', authenticate, async (req, res) => {
  const record = await prisma.capexPlan.update({ where: { id: req.params.id }, data: req.body })
  res.json(record)
})
