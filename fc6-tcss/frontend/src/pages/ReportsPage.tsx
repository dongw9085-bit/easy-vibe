...
export default function ReportsPage() {
  ...
  const waterfallOption = {
    ...
    series: [
      { name: 'EBIT', type: 'bar', data: fcData.map(e => e.ebit), itemStyle: { color: (p: any) => p.value >= 0 ? '#10b981' : '#ef4444' } },
      { name: 'EBIT率', type: 'line', yAxisIndex: 0, data: fcData.map(e => e.ebit), smooth: true, lineStyle: { color: '#3b82f6' } }
    ]
  }

  const revenueVsExpenseOption = {
    tooltip: { trigger: 'axis' },
  ...
  const marginRadarOption = {
    tooltip: {},
    radar: {
      indicator: fcData.map(e => ({ name: BU_LABELS[e.bu], max: 100 }))
    },
    series: [{
      type: 'radar',
      data: [{ value: fcData.map(e => e.grossMarginPct), name: '毛利率%' }],
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => window.open(`/api/reports/${id}/excel`, '_blank')} className="btn-primary flex items-center gap-2">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: '公司总收入 (FC H2)', value: fmtMoney(totalRevenue), sub: '7-12月预测' },
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                {['业务单元', '预测收入', '毛利润', '毛利率', '费用', 'EBIT', 'EBIT率'].map(h => <th key={h} className="table-th">{h}</th>)}
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {fcData.map(e => (
                <tr key={e.bu} className="hover:bg-gray-50">
                  ...
                  <td className="table-td">
                    <span className={`font-semibold ${e.ebit >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {e.ebit >= 0 ? <TrendingUp className="inline w-3.5 h-3.5 mr-1" /> : <TrendingDown className="inline w-3.5 h-3.5 mr-1" />}
                      {fmtMoney(e.ebit)}
                    </span>
                  </td>
                  <td className="table-td">
                    <span className={`badge ${Number(e.ebitPct) >= 10 ? 'bg-green-100 text-green-700' : Number(e.ebitPct) >= 0 ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700'}`}>
                      {e.ebitPct}%
                    </span>
                  </td>
                </tr>
              ...
              <tr className="bg-gray-50 font-semibold">
                ...
                <td className="table-td text-red-600">{fmtMoney(fcData.reduce((s, e) => s + e.expenses, 0))}</td>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="card">
          <h2 className="mb-4">收入 / 毛利 / 费用 / EBIT 对比</h2>
          <ReactECharts option={revenueVsExpenseOption} style={{ height: 300 }} />
        </div>
        <div className="card">
          <h2 className="mb-4">各 BU 毛利率雷达图</h2>
          <ReactECharts option={marginRadarOption} style={{ height: 300 }} />
        </div>
      </div>
      ...
      {ytdData.length > 0 && (
        <div className="card">
          <h2 className="mb-4">YTD 实际 vs FC 预测对比</h2>
          <ReactECharts option={{
            tooltip: { trigger: 'axis' },
            legend: { data: ['YTD实际收入', 'FC预测收入'], bottom: 0 },
            xAxis: { type: 'category', data: [...new Set([...ytdData, ...fcData].map(e => BU_LABELS[e.bu]))] },
            yAxis: { type: 'value', axisLabel: { formatter: (v: number) => `${(v / 10000).toFixed(0)}万` } },
            series: [
              { name: 'YTD实际收入', type: 'bar', data: ytdData.map(e => e.revenue), itemStyle: { color: '#94a3b8' } },
              { name: 'FC预测收入', type: 'bar', data: fcData.map(e => e.revenue), itemStyle: { color: '#3b82f6' } }
            ]
          }} style={{ height: 280 }} />
        </div>
      )}
